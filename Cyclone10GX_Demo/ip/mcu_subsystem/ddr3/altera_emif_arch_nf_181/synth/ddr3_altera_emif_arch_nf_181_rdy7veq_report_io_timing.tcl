...
source "$script_dir/ddr3_altera_emif_arch_nf_181_rdy7veq_report_timing_core.tcl"


if [ info exists ddr_db ] {
   unset ddr_db
}
ddr3_altera_emif_arch_nf_181_rdy7veq_initialize_ddr_db ddr_db var
...
foreach inst $instances {
   ...
   if {[llength $instances] <= 1} {
      set fbasename "${::GLOBAL_ddr3_altera_emif_arch_nf_181_rdy7veq_corename}"
   } else {
      set fbasename "${::GLOBAL_ddr3_altera_emif_arch_nf_181_rdy7veq_corename}_${inst_id}"
   }
   ...
   ddr3_altera_emif_arch_nf_181_rdy7veq_perform_read_capture_analysis $opcname $inst pins var summary
   
   #######################################
   # Write Analysis

   ddr3_altera_emif_arch_nf_181_rdy7veq_perform_write_launch_analysis $opcname $inst pins var summary

   #######################################
   # Address/command Analyses

   ddr3_altera_emif_arch_nf_181_rdy7veq_perform_ac_analysis  $opcname $inst pins var summary

   #######################################
   # DQS Gating Analysis

   ddr3_altera_emif_arch_nf_181_rdy7veq_perform_dqs_gating_analysis   $opcname $inst pins var summary

   #######################################
   # Write Levelling Analysis 
   
   ddr3_altera_emif_arch_nf_181_rdy7veq_perform_write_levelling_analysis $opcname $inst pins var summary

   #######################################
   # PHY Analyses
   
   ddr3_altera_emif_arch_nf_181_rdy7veq_perform_core_analysis $opcname $inst pins var summary

   #######################################
   # Print out the Summary Panel for this instance   

   set summary [lsort -command ddr3_altera_emif_arch_nf_181_rdy7veq_sort_proc $summary]
   ...
   puts $f "Core: ${::GLOBAL_ddr3_altera_emif_arch_nf_181_rdy7veq_corename} - Instance: $inst"
   puts $f "Path, Setup Margin, Hold Margin"

   
   post_message -type info "Core: ${::GLOBAL_ddr3_altera_emif_arch_nf_181_rdy7veq_corename} - Instance: $inst"
   ...
   foreach summary_line $summary {
      ...
      if {$su != "--"} {
         set su [ ddr3_altera_emif_arch_nf_181_rdy7veq_round_3dp $su]
      }
      if {$hold != "--"} {
         set hold [ ddr3_altera_emif_arch_nf_181_rdy7veq_round_3dp $hold]
      }
