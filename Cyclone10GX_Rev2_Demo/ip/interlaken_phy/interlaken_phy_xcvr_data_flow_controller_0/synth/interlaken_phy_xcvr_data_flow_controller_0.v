// interlaken_phy_xcvr_data_flow_controller_0.v

// Generated using ACDS version 18.1.2 277

`timescale 1 ps / 1 ps
module interlaken_phy_xcvr_data_flow_controller_0 (
		input  wire [7:0]  csr_s0_address,           //                   csr_s0.address
		input  wire        csr_s0_read,              //                         .read
		output wire [31:0] csr_s0_readdata,          //                         .readdata
		output wire        csr_s0_readdatavalid,     //                         .readdatavalid
		input  wire        csr_s0_write,             //                         .write
		input  wire [31:0] csr_s0_writedata,         //                         .writedata
		output wire        csr_s0_waitrequest,       //                         .waitrequest
		input  wire [63:0] rx_parallel_data,         //         rx_parallel_data.rx_parallel_data
		output wire [63:0] tx_parallel_data,         //         tx_parallel_data.tx_parallel_data
		input  wire        rx_enh_fifo_full,         //         rx_enh_fifo_full.rx_enh_fifo_full
		input  wire        rx_enh_fifo_pfull,        //        rx_enh_fifo_pfull.rx_enh_fifo_pfull
		input  wire        rx_enh_fifo_empty,        //        rx_enh_fifo_empty.rx_enh_fifo_empty
		input  wire        rx_enh_fifo_pempty,       //       rx_enh_fifo_pempty.rx_enh_fifo_pempty
		input  wire        tx_enh_fifo_pempty,       //       tx_enh_fifo_pempty.tx_enh_fifo_pempty
		input  wire        tx_enh_fifo_empty,        //        tx_enh_fifo_empty.tx_enh_fifo_empty
		input  wire        tx_enh_fifo_pfull,        //        tx_enh_fifo_pfull.tx_enh_fifo_pfull
		input  wire        tx_enh_fifo_full,         //         tx_enh_fifo_full.tx_enh_fifo_full
		output wire [31:0] avm_m0_address,           //                   avm_m0.address
		output wire        avm_m0_read,              //                         .read
		input  wire        avm_m0_waitrequest,       //                         .waitrequest
		input  wire [31:0] avm_m0_readdata,          //                         .readdata
		input  wire        avm_m0_readdatavalid,     //                         .readdatavalid
		output wire        avm_m0_write,             //                         .write
		output wire [31:0] avm_m0_writedata,         //                         .writedata
		output wire [2:0]  tx_control,               //               tx_control.tx_control
		input  wire [9:0]  rx_control,               //               rx_control.rx_control
		input  wire        reset,                    //                      rst.reset
		input  wire        clock,                    //                      clk.clk
		input  wire        tx_ready,                 //                 tx_ready.tx_ready
		input  wire        rx_ready,                 //                 rx_ready.rx_ready
		output wire        rx_irq,                   //                   irq_rx.irq
		output wire        tx_enh_data_valid,        //        tx_enh_data_valid.tx_enh_data_valid
		output wire        tx_irq,                   //                   irq_tx.irq
		input  wire        tx_coreclkin,             //             tx_coreclkin.clk
		input  wire        rx_coreclkin,             //             rx_coreclkin.clk
		output wire        rx_enh_fifo_rd_en,        //        rx_enh_fifo_rd_en.rx_enh_fifo_rd_en
		output wire        tx_err_ins,               //               tx_err_ins.tx_err_ins
		output wire        reset_phy,                //                reset_phy.reset
		output wire        pll_locked_out,           //           pll_locked_out.pll_locked
		input  wire        pll_powerdown_input,      //      pll_powerdown_input.pll_powerdown
		output wire        atx_pll_powerdown,        //        atx_pll_powerdown.pll_powerdown
		input  wire        atx_pll_locked,           //           atx_pll_locked.pll_locked
		output wire        rx_set_locktodata,        //        rx_set_locktodata.rx_set_locktodata
		output wire        rx_set_locktoref,         //         rx_set_locktoref.rx_set_locktoref
		output wire        tx_enh_frame_burst_en,    //    tx_enh_frame_burst_en.tx_enh_frame_burst_en
		output wire [1:0]  tx_enh_frame_diag_status, // tx_enh_frame_diag_status.tx_enh_frame_diag_status
		output wire [13:0] unused_tx_control,        //        unused_tx_control.unused_tx_control
		output wire [63:0] unused_tx_parallel_data,  //  unused_tx_parallel_data.unused_tx_parallel_data
		output wire        rx_seriallpbken,          //          rx_seriallpbken.rx_seriallpbken
		output wire        pll_select,               //               pll_select.pll_select
		input  wire [1:0]  rx_enh_frame_diag_status  // rx_enh_frame_diag_status.rx_enh_frame_diag_status
	);

	xcvr_data_flow_controller xcvr_data_flow_controller_0 (
		.csr_s0_address           (csr_s0_address),           //   input,   width = 8,                   csr_s0.address
		.csr_s0_read              (csr_s0_read),              //   input,   width = 1,                         .read
		.csr_s0_readdata          (csr_s0_readdata),          //  output,  width = 32,                         .readdata
		.csr_s0_readdatavalid     (csr_s0_readdatavalid),     //  output,   width = 1,                         .readdatavalid
		.csr_s0_write             (csr_s0_write),             //   input,   width = 1,                         .write
		.csr_s0_writedata         (csr_s0_writedata),         //   input,  width = 32,                         .writedata
		.csr_s0_waitrequest       (csr_s0_waitrequest),       //  output,   width = 1,                         .waitrequest
		.rx_parallel_data         (rx_parallel_data),         //   input,  width = 64,         rx_parallel_data.rx_parallel_data
		.tx_parallel_data         (tx_parallel_data),         //  output,  width = 64,         tx_parallel_data.tx_parallel_data
		.rx_enh_fifo_full         (rx_enh_fifo_full),         //   input,   width = 1,         rx_enh_fifo_full.rx_enh_fifo_full
		.rx_enh_fifo_pfull        (rx_enh_fifo_pfull),        //   input,   width = 1,        rx_enh_fifo_pfull.rx_enh_fifo_pfull
		.rx_enh_fifo_empty        (rx_enh_fifo_empty),        //   input,   width = 1,        rx_enh_fifo_empty.rx_enh_fifo_empty
		.rx_enh_fifo_pempty       (rx_enh_fifo_pempty),       //   input,   width = 1,       rx_enh_fifo_pempty.rx_enh_fifo_pempty
		.tx_enh_fifo_pempty       (tx_enh_fifo_pempty),       //   input,   width = 1,       tx_enh_fifo_pempty.tx_enh_fifo_pempty
		.tx_enh_fifo_empty        (tx_enh_fifo_empty),        //   input,   width = 1,        tx_enh_fifo_empty.tx_enh_fifo_empty
		.tx_enh_fifo_pfull        (tx_enh_fifo_pfull),        //   input,   width = 1,        tx_enh_fifo_pfull.tx_enh_fifo_pfull
		.tx_enh_fifo_full         (tx_enh_fifo_full),         //   input,   width = 1,         tx_enh_fifo_full.tx_enh_fifo_full
		.avm_m0_address           (avm_m0_address),           //  output,  width = 32,                   avm_m0.address
		.avm_m0_read              (avm_m0_read),              //  output,   width = 1,                         .read
		.avm_m0_waitrequest       (avm_m0_waitrequest),       //   input,   width = 1,                         .waitrequest
		.avm_m0_readdata          (avm_m0_readdata),          //   input,  width = 32,                         .readdata
		.avm_m0_readdatavalid     (avm_m0_readdatavalid),     //   input,   width = 1,                         .readdatavalid
		.avm_m0_write             (avm_m0_write),             //  output,   width = 1,                         .write
		.avm_m0_writedata         (avm_m0_writedata),         //  output,  width = 32,                         .writedata
		.tx_control               (tx_control),               //  output,   width = 3,               tx_control.tx_control
		.rx_control               (rx_control),               //   input,  width = 10,               rx_control.rx_control
		.reset                    (reset),                    //   input,   width = 1,                      rst.reset
		.clock                    (clock),                    //   input,   width = 1,                      clk.clk
		.tx_ready                 (tx_ready),                 //   input,   width = 1,                 tx_ready.tx_ready
		.rx_ready                 (rx_ready),                 //   input,   width = 1,                 rx_ready.rx_ready
		.rx_irq                   (rx_irq),                   //  output,   width = 1,                   irq_rx.irq
		.tx_enh_data_valid        (tx_enh_data_valid),        //  output,   width = 1,        tx_enh_data_valid.tx_enh_data_valid
		.tx_irq                   (tx_irq),                   //  output,   width = 1,                   irq_tx.irq
		.tx_coreclkin             (tx_coreclkin),             //   input,   width = 1,             tx_coreclkin.clk
		.rx_coreclkin             (rx_coreclkin),             //   input,   width = 1,             rx_coreclkin.clk
		.rx_enh_fifo_rd_en        (rx_enh_fifo_rd_en),        //  output,   width = 1,        rx_enh_fifo_rd_en.rx_enh_fifo_rd_en
		.tx_err_ins               (tx_err_ins),               //  output,   width = 1,               tx_err_ins.tx_err_ins
		.reset_phy                (reset_phy),                //  output,   width = 1,                reset_phy.reset
		.pll_locked_out           (pll_locked_out),           //  output,   width = 1,           pll_locked_out.pll_locked
		.pll_powerdown_input      (pll_powerdown_input),      //   input,   width = 1,      pll_powerdown_input.pll_powerdown
		.atx_pll_powerdown        (atx_pll_powerdown),        //  output,   width = 1,        atx_pll_powerdown.pll_powerdown
		.atx_pll_locked           (atx_pll_locked),           //   input,   width = 1,           atx_pll_locked.pll_locked
		.rx_set_locktodata        (rx_set_locktodata),        //  output,   width = 1,        rx_set_locktodata.rx_set_locktodata
		.rx_set_locktoref         (rx_set_locktoref),         //  output,   width = 1,         rx_set_locktoref.rx_set_locktoref
		.tx_enh_frame_burst_en    (tx_enh_frame_burst_en),    //  output,   width = 1,    tx_enh_frame_burst_en.tx_enh_frame_burst_en
		.tx_enh_frame_diag_status (tx_enh_frame_diag_status), //  output,   width = 2, tx_enh_frame_diag_status.tx_enh_frame_diag_status
		.unused_tx_control        (unused_tx_control),        //  output,  width = 14,        unused_tx_control.unused_tx_control
		.unused_tx_parallel_data  (unused_tx_parallel_data),  //  output,  width = 64,  unused_tx_parallel_data.unused_tx_parallel_data
		.rx_seriallpbken          (rx_seriallpbken),          //  output,   width = 1,          rx_seriallpbken.rx_seriallpbken
		.pll_select               (pll_select),               //  output,   width = 1,               pll_select.pll_select
		.rx_enh_frame_diag_status (rx_enh_frame_diag_status)  //   input,   width = 2, rx_enh_frame_diag_status.rx_enh_frame_diag_status
	);

endmodule
