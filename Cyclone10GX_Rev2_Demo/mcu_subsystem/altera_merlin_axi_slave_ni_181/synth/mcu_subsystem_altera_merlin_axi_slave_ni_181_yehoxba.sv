...
module mcu_subsystem_altera_merlin_axi_slave_ni_181_yehoxba
#(
    // -----------------------------------------------
...
(
   ...
   endgenerate
    ...
    generate if(ACE_LITE_SUPPORT == 0) begin
       ...
       assign write_cmd_domain         = DOMAIN_VALUE[1:0];
       assign write_cmd_snoop          = SNOOP_VALUE[1:0];
       assign write_cmd_bar            = BARRIER_VALUE[2:0];
       assign write_cmd_unique         = WUNIQUE_VALUE[0]; 
       assign write_cmd_cache          = {CACHE_W{1'b0}}; 
    end
    endgenerate
    ...
    generate if(ACE_LITE_SUPPORT == 0) begin
       ...
       assign read_cmd_domain          = DOMAIN_VALUE[1:0];
       assign read_cmd_snoop           = SNOOP_VALUE[1:0];
       assign read_cmd_bar             = BARRIER_VALUE[3:0];
       assign read_cmd_cache           = {CACHE_W{1'b0}}; 
    end
    endgenerate
...
// start of packet assertion should only happen at the start of burst and should not be asserted at the start of sub-burst
  ...
  if (SYNC_RESET == 0) begin : async_fifo
    ...
    mcu_subsystem_altera_merlin_axi_slave_ni_181_evxcnfa #(
        .SYMBOLS_PER_BEAT    (1),
    ...
    mcu_subsystem_altera_merlin_axi_slave_ni_181_lq2z2di # (
        .IMPL                  ("reg"),
        .SYMBOLS_PER_BEAT      (1),   
        .BITS_PER_SYMBOL       (ST_DATA_W + 1), 
        .FIFO_DEPTH            (WRITE_ACCEPTANCE_CAPABILITY), 
        ...
        .EMPTY_LATENCY         (1), 
        .SHOWAHEAD             (1), 
        .ALMOST_FULL_THRESHOLD (WRITE_ACCEPTANCE_CAPABILITY) 
    ) write_rsp_fifo (
       .clk               (aclk),                                 // clk.clk
       .reset             (areset),                               // clk_reset.reset
       .in_data           (write_rsp_fifo_indata),                // in.data
       .in_valid          (write_rsp_fifo_invalid),               // .valid
       .in_ready          (write_rsp_fifo_inready),               // .ready
       .out_data          (write_rsp_fifo_outdata),               // out.data
       .out_valid         (write_rsp_fifo_outvalid),              // .valid
       .out_ready         (write_rsp_fifo_outready)               // .ready
    );
    ...
    mcu_subsystem_altera_merlin_axi_slave_ni_181_lq2z2di # (
        ...
        .EMPTY_LATENCY         (1), 
        .SHOWAHEAD             (1), 
        .ALMOST_FULL_THRESHOLD (READ_ACCEPTANCE_CAPABILITY) 
    ) read_rsp_fifo (
       .clk               (aclk),                                 // clk.clk
       .reset             (areset),                              // clk_reset.reset
       .in_data           (read_rsp_fifo_indata),                 // in.data
       .in_valid          (read_rsp_fifo_invalid),                // .valid
       .in_ready          (read_rsp_fifo_inready),                // .ready
       .out_data          (read_rsp_fifo_outdata),                // out.data
       .out_valid         (read_rsp_fifo_outvalid),               // .valid
       .out_ready         (read_rsp_fifo_outready)                // .ready
    );
